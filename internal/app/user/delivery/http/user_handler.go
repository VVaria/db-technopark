...
func (uh *UserHandler) UserCreateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if errE != nil {
		...
		w.Write(errors.JSONMessage("Пользователь уже присутсвует в базе данных.", userData))
		return
	}

	w.WriteHeader(http.StatusCreated)
	w.Write(errors.JSONMessage(" Пользователь успешно создан.", user))
}
...
func (uh *UserHandler) UserGetProfileHandler(w http.ResponseWriter, r *http.Request) {
	...
	w.Write(errors.JSONMessage("Информация о пользователе.", user))
}
...
func (uh *UserHandler) UserChangeProfileHandler(w http.ResponseWriter, r *http.Request) {
	...
	w.WriteHeader(http.StatusOK)
	w.Write(errors.JSONMessage("Актуальная информация о пользователе после изменения профиля.", user))
}
